...
function handleGetResultsById(Request $request, Response $response, array $args) {
    ...
    if (isset($resultID)) {
        ...
        $results_info = $result_model->getResultById($resultID);
        if (!$results_info) {
            // No matches found?
...
function handleDeleteResultsById(Request $request, Response $response, array $data){
    $results_info = array();
    $response_data = array();
    $response_code = HTTP_OK;
    $result_model = new FinalResultsModel();
    ...
    if ($requested_format[0] === APP_MEDIA_TYPE_JSON) {
        ...
        $response_data = "Deleted Fight Results Succesfully";
    } else {
        $response_data = json_encode(getErrorUnsupportedFormat());
        $response_code = HTTP_UNSUPPORTED_MEDIA_TYPE;
    }
